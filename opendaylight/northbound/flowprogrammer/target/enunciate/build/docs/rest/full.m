...
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  ...
  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    ...
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element flowConfigs."];
    }
  }

  if (xmlStrcmp(BAD_CAST "flowConfigs", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}flowConfigs.");
#endif
    _flowConfigs = (FULLNS0FlowConfigs *)[FULLNS0FlowConfigs readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}flowConfigs.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read FULLNS0FlowConfigs. Expected element flowConfigs. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read FULLNS0FlowConfigs. Expected element flowConfigs. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this FULLNS0FlowConfigs to XML under element name "flowConfigs".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "flowConfigs", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}flowConfigs. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}flowConfigs for root element {}flowConfigs...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}flowConfigs for root element {}flowConfigs...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}flowConfigs. XML writer status: %i\n", rc];
  }
}
