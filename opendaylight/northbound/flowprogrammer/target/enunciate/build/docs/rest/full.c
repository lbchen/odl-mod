...
/**
 * Reads a FlowConfigs element from XML. The element to be read is "flowConfigs", and
...
/**
 * Writes a FlowConfigs to XML under element name "flowConfigs".
 *
 ...
 * @return The FlowConfigs, or NULL in case of error.
 */
struct full_ns0_flowConfigs *xmlTextReaderReadNs0FlowConfigsElement(xmlTextReaderPtr reader);
 ...
 * @return The bytes written (may be 0 in case of buffering) or -1 in case of error.
 */
static int xmlTextWriterWriteNs0FlowConfigsElement(xmlTextWriterPtr writer, struct full_ns0_flowConfigs *_flowConfigs);
 ...
 * @return The bytes written (may be 0 in case of buffering) or -1 in case of error.
 */
static int xmlTextWriterWriteNs0FlowConfigsElementNS(xmlTextWriterPtr writer, struct full_ns0_flowConfigs *_flowConfigs, int writeNamespaces);
 ...
 */
static void freeNs0FlowConfigsElement(struct full_ns0_flowConfigs *_flowConfigs);
...
struct full_ns0_flowConfigs *xml_read_full_ns0_flowConfigs(xmlTextReaderPtr reader) {
  ...
  return xmlTextReaderReadNs0FlowConfigsElement(reader);
}
 ...
 */
int xml_write_full_ns0_flowConfigs(xmlTextWriterPtr writer, struct full_ns0_flowConfigs *_flowConfigs) {
  return xmlTextWriterWriteNs0FlowConfigsElementNS(writer, _flowConfigs, 1);
}
...
struct full_ns0_flowConfigs *xmlTextReaderReadNs0FlowConfigsElement(xmlTextReaderPtr reader) {
  ...
  if (xmlTextReaderNodeType(reader) == XML_READER_TYPE_ELEMENT
    && xmlStrcmp(BAD_CAST "flowConfigs", xmlTextReaderConstLocalName(reader)) == 0
    && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    printf("Attempting to read root element {}flowConfigs.\n");
#endif
  ...
  if (_flowConfigs == NULL) {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      printf("attempt to read {}flowConfigs failed. current element: {}%s\n",  xmlTextReaderConstLocalName(reader));
    }
    else {
      printf("attempt to read {}flowConfigs failed. current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader));
    }
  }
#endif
 ...
 */
static int xmlTextWriterWriteNs0FlowConfigsElement(xmlTextWriterPtr writer, struct full_ns0_flowConfigs *_flowConfigs) {
  return xmlTextWriterWriteNs0FlowConfigsElementNS(writer, _flowConfigs, 0);
}
...
static int xmlTextWriterWriteNs0FlowConfigsElementNS(xmlTextWriterPtr writer, struct full_ns0_flowConfigs *_flowConfigs, int writeNamespaces) {
  ...
  status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "flowConfigs", NULL);
  if (status < 0) {
#if DEBUG_ENUNCIATE
    printf("unable to write start element {}flowConfigs. status: %i\n", status);
#endif
    return status;
  }
  totalBytes += status;
...
#if DEBUG_ENUNCIATE > 1
  printf("writing type {}flowConfigs for root element {}flowConfigs...\n");
#endif
  status = xmlTextWriterWriteNs0FlowConfigsType(writer, _flowConfigs);
  if (status < 0) {
#if DEBUG_ENUNCIATE
    printf("unable to write type for start element {}flowConfigs. status: %i\n", status);
#endif
    return status;
  }
  totalBytes += status;
  ...
  if (status < 0) {
#if DEBUG_ENUNCIATE
    printf("unable to end element {}flowConfigs. status: %i\n", status);
#endif
    return status;
  }
  totalBytes += status;
