...
/**
 * Reads a StaticRoutes element from XML. The element to be read is "staticRoutes", and
 * it is assumed that the reader is pointing to the XML document (not the element).
 *
 * @param reader The XML reader.
...
/**
 * Writes a StaticRoutes to XML under element name "staticRoutes".
 *
 * @param writer The XML writer.
 ...
 * @return The StaticRoutes, or NULL in case of error.
 */
struct full_ns0_staticRoutes *xmlTextReaderReadNs0StaticRoutesElement(xmlTextReaderPtr reader);
 ...
 * @return The bytes written (may be 0 in case of buffering) or -1 in case of error.
 */
static int xmlTextWriterWriteNs0StaticRoutesElement(xmlTextWriterPtr writer, struct full_ns0_staticRoutes *_staticRoutes);
 ...
 * @return The bytes written (may be 0 in case of buffering) or -1 in case of error.
 */
static int xmlTextWriterWriteNs0StaticRoutesElementNS(xmlTextWriterPtr writer, struct full_ns0_staticRoutes *_staticRoutes, int writeNamespaces);
 ...
 */
static void freeNs0StaticRoutesElement(struct full_ns0_staticRoutes *_staticRoutes);
...
struct full_ns0_staticRoutes *xml_read_full_ns0_staticRoutes(xmlTextReaderPtr reader) {
  int status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
  return xmlTextReaderReadNs0StaticRoutesElement(reader);
}
 ...
 */
int xml_write_full_ns0_staticRoutes(xmlTextWriterPtr writer, struct full_ns0_staticRoutes *_staticRoutes) {
  return xmlTextWriterWriteNs0StaticRoutesElementNS(writer, _staticRoutes, 1);
}
...
struct full_ns0_staticRoutes *xmlTextReaderReadNs0StaticRoutesElement(xmlTextReaderPtr reader) {
  ...
  if (xmlTextReaderNodeType(reader) == XML_READER_TYPE_ELEMENT
    && xmlStrcmp(BAD_CAST "staticRoutes", xmlTextReaderConstLocalName(reader)) == 0
    && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    printf("Attempting to read root element {}staticRoutes.\n");
#endif
  ...
  if (_staticRoutes == NULL) {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      printf("attempt to read {}staticRoutes failed. current element: {}%s\n",  xmlTextReaderConstLocalName(reader));
    }
    else {
      printf("attempt to read {}staticRoutes failed. current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader));
    }
  }
#endif
 ...
 */
static int xmlTextWriterWriteNs0StaticRoutesElement(xmlTextWriterPtr writer, struct full_ns0_staticRoutes *_staticRoutes) {
  return xmlTextWriterWriteNs0StaticRoutesElementNS(writer, _staticRoutes, 0);
}
...
static int xmlTextWriterWriteNs0StaticRoutesElementNS(xmlTextWriterPtr writer, struct full_ns0_staticRoutes *_staticRoutes, int writeNamespaces) {
  ...
  status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "staticRoutes", NULL);
  if (status < 0) {
#if DEBUG_ENUNCIATE
    printf("unable to write start element {}staticRoutes. status: %i\n", status);
#endif
    return status;
  }
  totalBytes += status;
...
#if DEBUG_ENUNCIATE > 1
  printf("writing type {}staticRoutes for root element {}staticRoutes...\n");
#endif
  status = xmlTextWriterWriteNs0StaticRoutesType(writer, _staticRoutes);
  if (status < 0) {
#if DEBUG_ENUNCIATE
    printf("unable to write type for start element {}staticRoutes. status: %i\n", status);
#endif
    return status;
  }
  totalBytes += status;
  ...
  if (status < 0) {
#if DEBUG_ENUNCIATE
    printf("unable to end element {}staticRoutes. status: %i\n", status);
#endif
    return status;
  }
  totalBytes += status;
