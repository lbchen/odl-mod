...
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  ...
  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    ...
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element staticRoutes."];
    }
  }

  if (xmlStrcmp(BAD_CAST "staticRoutes", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}staticRoutes.");
#endif
    _staticRoutes = (FULLNS0StaticRoutes *)[FULLNS0StaticRoutes readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}staticRoutes.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read FULLNS0StaticRoutes. Expected element staticRoutes. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read FULLNS0StaticRoutes. Expected element staticRoutes. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this FULLNS0StaticRoutes to XML under element name "staticRoutes".
...
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "staticRoutes", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}staticRoutes. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}staticRoutes for root element {}staticRoutes...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}staticRoutes for root element {}staticRoutes...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}staticRoutes. XML writer status: %i\n", rc];
  }
}
