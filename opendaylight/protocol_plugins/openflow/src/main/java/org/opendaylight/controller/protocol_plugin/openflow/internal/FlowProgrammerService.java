...
public class FlowProgrammerService implements IPluginInFlowProgrammerService,
    ...
    private void enQueueConfig(List<Action> actions){
    	for(Action action:actions){
    		String type = action.toString().split(":")[0];
    		if(type == "enqueue"){
...
//            	enQueueConfig(actions);
//            	Enqueue eq = new Enqueue();
//            	eq.setPort((short)3);
//            	eq.setQueueid(1);
//            	actions.add(eq);
//            	flow.setActions(actions);
                ...
                OFMessage qmsg = new OFQueueConfigRequest();
                ((OFQueueConfigRequest)qmsg).setPort((short)3);
                ((OFQueueConfigRequest)qmsg).setLength(U16.t(OFQueueConfigRequest.MINIMUM_LENGTH));
                //((OFQueueConfigRequest)qmsg).setType(OFType.QUEUE_CONFIG_REQUEST);
                Object result;
                if (rid == 0) {
                    /*
