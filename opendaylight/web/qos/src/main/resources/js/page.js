    ...
    id : {
        ...
        modal : {
            ...
            action : {
                ...
                // add,lbc,0726
                qport : "one_f_flows_id_modal_action_qport",
                queue : "one_f_flows_id_modal_action_queue",
        ...
        action : {
            parse : function(option) {
                switch (option) {
                    ...
                    case "enqueue":
                    	var h3 = "EnQueue";
                        var $modal = one.f.flows.modal.action.initialize(h3, one.f.flows.modal.action.body.enqueue, one.f.flows.modal.action.add.enqueue);
                        $modal.modal();
                        break;
                    case "setVlanId" :
                        var h3 = "Set VLAN ID";
            ...
            enqueue : function(h3, bodyCallback, queueCallback) {
            	var footer = one.f.flows.modal.action.footer();
            	var $body = bodyCallback();
            	var $modal = one.lib.modal.spawn(one.f.flows.id.modal.action.modal,h3,$body,footer);
            	$('#'+one.f.flows.id.modal.action.close, $modal).click(function() {
            ...
            add : {
                ...
                // {enqueue,lbc,0726}
                enqueue : function($modal){
                	var $qport = $('#'+one.f.flows.id.modal.action.qport).find('option:selected');
                	var $queues = $('#'+one.f.flows.id.modal.action.queue).find('option:selected');
                	var port = '';
                	var queue = '';
                	$qport.each(function(index,value) {
                		port = port+$(value).attr('value');
                	});
//                	$queues.each(function(index,value) {
//                		queue = queue+$(value).text();
//                	});
                	queue = $queues.text();
                	port = $qport.text();
                	//queue = queue.slice(0,-1);
                	var $tr = one.f.flows.modal.action.table.add("EnQueue",port+":"+queue);
                	$tr.attr('id','enqueue');
                	$tr.data('action','ENQUEUE='+port+":"+queue);
                	one.f.flows.modal.action.table.append($tr);
                	$modal.modal('hide');
				...
				modal : {
					...
					footer : function() {
						...
						footer.push($cancelButton);table
            ...
            body : {
                ...
                enqueue: function(){       
                	var $form = $(document.createElement('form'));
                	var $portset = $(document.createElement('portset'));
                	var $queueset = $(document.createElement('queueset'));
                	$label = one.lib.form.label("Select Port");
                	$label2 = one.lib.form.label("Select Queue");
                    var ports = one.f.flows.registry.nodeports[one.f.flows.registry.currentNode]['ports'];
                    var queues = ["1","2","3"];
                    $select = one.lib.form.select.create(ports, true);
                    $select2 = one.lib.form.select.create(queues, true);
                    $select.attr('id', one.f.flows.id.modal.action.qport);
                    $select2.attr('id', one.f.flows.id.modal.action.queue);
                    $portset.append($label).append($select);
                    $queueset.append($label2).append($select2);
                    $form.append($portset);
                    $form.append($queueset);
                    return $form;
                },
